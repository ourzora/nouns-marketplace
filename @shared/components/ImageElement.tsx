import { Box, BoxProps } from '@zoralabs/zord'
import { useCallback, useEffect, useState } from 'react'
import { fullSizeImage } from '@shared/SharedStyles.css'
import { useInView } from 'react-intersection-observer'

const placeHolder = `data:image/png;base64,`

export interface ImageElementProps extends BoxProps {
  /** Image to be displayed as a lazy loaded image element */
  src?: string
  /** Image will scale to wrapper component
   * - don't forget to set either an aspect ration or w/h and make the parent div pos relative
   * Default set to true
   * */
  fitImage?: boolean
  /** String value for transition speed */
  transitionCss?: string
}

export function ImageElement({
  src,
  fitImage = true,
  transitionCss = `all 250ms ease-in`,
  ...props
}: ImageElementProps) {
  const [loaded, setLoaded] = useState(false)
  const [scrolled, setScrolled] = useState(false)

  const loadedHandler = useCallback((e) => {
    if (e.target.src !== placeHolder) setLoaded(true)
  }, [])

  const { ref, inView } = useInView({
    threshold: 0,
  })

  useEffect(() => {
    if (inView) setScrolled(true)
  }, [inView])

  return (
    <Box
      {...props}
      as="img"
      ref={ref}
      src={scrolled ? src : placeHolder}
      onLoad={loadedHandler}
      style={{
        opacity: loaded ? 1 : 0,
        transition: transitionCss,
        willChange: 'all',
      }}
      className={[
        {
          [fullSizeImage]: fitImage,
        },
      ]}
    />
  )
}
