...
Zord is a general purpose UI library built on top of [Vanilla Extract](https://vanilla-extract.style/). Currently Zord is in alpha and used extensively through Zora's frontends. More robust documentation is in the works along with theming options. You will find that it is a extremely robust library, that does require knowledge of vanilla extract to extend... so check the [docs](https://vanilla-extract.style/documentation/getting-started).
